...
- **全体シェル（確定）: 左＝ワークスペース・サイドバー（LoRA プロジェクト一覧・切替＋新規作成）
  ／右＝メイン領域**。サイドバー 1 項目＝1 LoRA プロジェクト（`projects` テーブル）。選択で
  アクティブプロジェクトを切替、右側がそれに追従。
- **ステッパー型**（Import → Tag → Curate → Build → Train → Test）。基本一方向のため
  ノードグラフより素直。メイン領域の上部に常時ステッパー。
- **タグエディタ（確定）: 上部＝サムネイル・グリッド／下部＝タグ編集バー**。グリッドで
  複数選択（Ctrl/Shift）→ 下部バーからタグ追加／削除・LLM 剪定を一括適用。剪定候補・
  ブラックリスト該当をハイライト、`keep` オフはグレーアウト、トリガーワードは先頭固定。
