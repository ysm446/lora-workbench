...
- **全体シェル（確定）**: **左＝ワークスペース・サイドバー（LoRA プロジェクト一覧・切替）／
  右＝メイン領域（上部ステッパー＋各ステップの作業画面）**。
  - サイドバーの 1 項目＝1 LoRA プロジェクト（`projects` テーブル／`projects/{project}/`）。
    選択したプロジェクトがアクティブになり、右側の内容がそれに切り替わる。
  - 下部「+ 新規」で LoRA プロジェクトを作成（`projects/{name}/` ＋ `project.db` を生成）。
  - プロジェクトごとに `base_model` / `gen_model` 等の設定を保持。
- **ステッパー型**（Import → Tag → Curate → Build → Train → Test）。この処理は基本的に一方向なのでノードグラフより素直。メイン領域の上部に常時ステッパーを置く。
- **タグエディタ（Tag/Curate 画面の確定レイアウト）**: **上部＝サムネイル・グリッド／下部＝タグ編集バー**。
  - グリッド: 剪定候補（高頻度タグ保有）やブラックリスト該当をハイライト。`keep` オフはグレーアウト。
  - 複数選択（Ctrl/Shift）→ 下部バーから選択画像へタグ追加／削除・LLM 剪定を **一括適用**。
  - 下部バー: 単一選択時はその画像のタグ編集、複数選択時は共通タグ＋一括操作。トリガーワードは常に先頭固定。
